...
cond_reg <- lm(log_y_total_m_ha ~ Mujer + poly(age, 2, raw = TRUE) + 
                 ...
                 Tamaño_firma + Full_time + formal + Oficio_cat, data = data)
...
set.seed(321)
result = boot(data, boot_stat, R = 10000)
result$t0 = round(result$t0, 4)
result$t = apply(result$t, 2, round, 4)
result
...
predictions_gender = function(data, indices, gender) {
  ...
  model = lm(log_y_total_m_ha ~ Mujer + poly(age, 2, raw = TRUE) + 
                Max_nivel_educacion2 + poly(Experiencia_emp_act, 2, raw = TRUE) + 
                Tamaño_firma + Full_time + formal + Oficio_cat, data = sample_data)
  ...
  pred_data = data.frame(
    ...
    Oficio_cat = names(which.max(table(sample_data$Oficio_cat )))  # Moda de oficio
  )
...
get_peak_age_bootstrap = function(data, indices, gender) {
  ...
  model = lm(log_y_total_m_ha ~ Mujer + poly(age, 2, raw = TRUE) + 
                Max_nivel_educacion2 + poly(Experiencia_emp_act, 2, raw = TRUE) + 
                Tamaño_firma + Full_time + formal + Oficio_cat, data = sample_data[sample_data$Mujer == gender, ])
